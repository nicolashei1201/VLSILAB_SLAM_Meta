...
const EAS_ICP::Transform& EAS_ICP::Register(const SourceCloud& srcCloud, const TargetCloud& tgtCloud, const Transform& initialGuess) {
  ...
  while (true) {
	...
	Eigen::Matrix<int, Eigen::Dynamic, 2, Eigen::RowMajor> corrs_temp1(cnttrans1, 2);
	cnttrans1 = 0;
	for(int i =0;i<corrs.rows(); i++)
	{
		if(cnt1[i]<1)
		{
			...
			corrs_temp1(cnttrans1, 0) = corrs(i,0);
			corrs_temp1(cnttrans1, 1) = corrs(i,1);
			cnttrans1++;
		}
    ...
    rt6D1 = MinimizingP2PLErrorMetric(transformedCloud1(corrs_temp1.col(0), Eigen::all), tgtCloud(corrs_temp1.col(1), Eigen::all), kinectNoiseWeights(corrs_temp1.col(0), Eigen::all));
	...
	Eigen::Matrix<int, Eigen::Dynamic, 2, Eigen::RowMajor> corrs_temp2(cnttrans2, 2);
	cnttrans2 = 0;
	for(int i =0;i<corrs.rows(); i++)
	{
		if(cnt2[i]<1)
		{
			corrs_temp2(cnttrans2, 0) = corrs(i,0);
			corrs_temp2(cnttrans2, 1) = corrs(i,1);
			cnttrans2++;
		}
    ...
    rt6D2 = MinimizingP2PLErrorMetric(transformedCloud(corrs_temp2.col(0), Eigen::all), tgtCloud(corrs_temp2.col(1), Eigen::all), kinectNoiseWeights(corrs_temp2.col(0), Eigen::all));
	...
	Eigen::Matrix<int, Eigen::Dynamic, 2, Eigen::RowMajor> corrs_temp3(cnttrans3, 2);
	cnttrans3 = 0;
	for(int i =0;i<corrs.rows(); i++)
	{
		if(cnt3[i]<1)
		{
			corrs_temp3(cnttrans3, 0) = corrs(i,0);
			corrs_temp3(cnttrans3, 1) = corrs(i,1);
			cnttrans3++;
		}
    ...
    rt6D3 = MinimizingP2PLErrorMetric(transformedCloud(corrs_temp3.col(0), Eigen::all), tgtCloud(corrs_temp3.col(1), Eigen::all), kinectNoiseWeights(corrs_temp3.col(0), Eigen::all));
	...
	Eigen::Matrix<int, Eigen::Dynamic, 2, Eigen::RowMajor> corrs_temp4(cnttrans4, 2);
	cnttrans4 = 0;
	for(int i =0;i<corrs.rows(); i++)
	{
		if(cnt4[i]<0.9)
		{
			corrs_temp4(cnttrans4, 0) = corrs(i,0);
			corrs_temp4(cnttrans4, 1) = corrs(i,1);
			cnttrans4++;
		}
    ...
    rt6D4 = MinimizingP2PLErrorMetric(transformedCloud(corrs_temp4.col(0), Eigen::all), tgtCloud(corrs_temp4.col(1), Eigen::all), kinectNoiseWeights(corrs_temp4.col(0), Eigen::all));
	// ++ meta task loop end
	// ++ pose fusion = mean(all tasks' rt6D)
    rt6D(0) = (rt6D1(0)+ rt6D2(0) +rt6D3(0) +rt6D4(0))/4;
	rt6D(1) = (rt6D1(1)+ rt6D2(1) +rt6D3(1) +rt6D4(1))/4;
    rt6D(2) = (rt6D1(2)+ rt6D2(2) +rt6D3(2) +rt6D4(2))/4;
	rt6D(3) = (rt6D1(3)+ rt6D2(3) +rt6D3(3) +rt6D4(3))/4;
	rt6D(4) = (rt6D1(4)+ rt6D2(4) +rt6D3(4) +rt6D4(4))/4;
	rt6D(5) = (rt6D1(5)+ rt6D2(5) +rt6D3(5) +rt6D4(5))/4;
