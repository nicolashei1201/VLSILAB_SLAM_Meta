...
const EAS_ICP::Transform& EAS_ICP::Register(const SourceCloud& srcCloud, const TargetCloud& tgtCloud, const Transform& initialGuess) {
  ...
  while (true) {
	...
	for(int i =0;i<srcCloud.rows(); i++)
	{
		if(cnt1[i]==1)
		{
			transformedCloud1(cnttrans1, 0) = transformedCloud(i, 0);
			transformedCloud1(cnttrans1, 1) = transformedCloud(i, 1);
			transformedCloud1(cnttrans1, 2) = transformedCloud(i, 2);
			transformedCloud1(cnttrans1, 3) = transformedCloud(i, 3);
			transformedCloud1(cnttrans1, 4) = transformedCloud(i, 4);
			transformedCloud1(cnttrans1, 5) = transformedCloud(i, 5);
    ...
    if (!MatchingByProject2DAndWalk1(transformedCloud1, tgtCloud)) {
    ...
    rt6D1 = MinimizingP2PLErrorMetric(transformedCloud1(corrs1.col(0), Eigen::all), tgtCloud(corrs1.col(1), Eigen::all), kinectNoiseWeights(corrs1.col(0), Eigen::all));
	std::cout<<"cloudnumber1"<<std::endl;
	std::cout<<srcCloud.rows()<<std::endl;
	std::cout<<"cnttrans1"<<std::endl;
	std::cout<<cnttrans1<<std::endl;
	meanweight1 = 1;
	}
	std::cout<<"corrs1num"<<std::endl;
	std::cout<<corrs1.rows()<<std::endl;
	//2nd set of correspondences
	int cnt2[srcCloud.rows()];
    for(int i=0; i<srcCloud.rows(); i++)
	{
		//random
	...
	for(int i =0;i<srcCloud.rows(); i++)
	{
		if(cnt2[i]==1)
		{
			transformedCloud2(cnttrans2, 0) = transformedCloud(i, 0);
			transformedCloud2(cnttrans2, 1) = transformedCloud(i, 1);
			transformedCloud2(cnttrans2, 2) = transformedCloud(i, 2);
			transformedCloud2(cnttrans2, 3) = transformedCloud(i, 3);
			transformedCloud2(cnttrans2, 4) = transformedCloud(i, 4);
			transformedCloud2(cnttrans2, 5) = transformedCloud(i, 5);
    ...
    Eigen::Vector<Scalar, 6> rt6D2;
		//match correspondence
    if (!MatchingByProject2DAndWalk2(transformedCloud2, tgtCloud)) {
    ...
    rt6D2 = MinimizingP2PLErrorMetric(transformedCloud2(corrs2.col(0), Eigen::all), tgtCloud(corrs2.col(1), Eigen::all), kinectNoiseWeights(corrs2.col(0), Eigen::all));
	std::cout<<"cloudnumber2"<<std::endl;
	std::cout<<srcCloud.rows()<<std::endl;
	std::cout<<"cnttrans2"<<std::endl;
	std::cout<<cnttrans2<<std::endl;
	meanweight2 = 1;
	}
	std::cout<<"corrs2num"<<std::endl;
	std::cout<<corrs2.rows()<<std::endl;
	//3rd set of correspondences
	int cnt3[srcCloud.rows()];
    for(int i=0; i<srcCloud.rows(); i++)
	{
		//random
	...
	for(int i =0;i<srcCloud.rows(); i++)
	{
		if(cnt3[i]==1)
		{
			transformedCloud3(cnttrans3, 0) = transformedCloud(i, 0);
			transformedCloud3(cnttrans3, 1) = transformedCloud(i, 1);
			transformedCloud3(cnttrans3, 2) = transformedCloud(i, 2);
			transformedCloud3(cnttrans3, 3) = transformedCloud(i, 3);
			transformedCloud3(cnttrans3, 4) = transformedCloud(i, 4);
			transformedCloud3(cnttrans3, 5) = transformedCloud(i, 5);
    ...
    Eigen::Vector<Scalar, 6> rt6D3;
		//match correspondence
    if (!MatchingByProject2DAndWalk3(transformedCloud3, tgtCloud)) {
		std::cout<<"jump3"<<std::endl;
		meanweight3 =0;
      //break; // when correspondence size less than 6
    }
	else{
	std::cout<<"cloudnumber3"<<std::endl;
	std::cout<<srcCloud.rows()<<std::endl;
	std::cout<<"cnttrans3"<<std::endl;
	std::cout<<cnttrans3<<std::endl;
    //get iteration transformation by minimizing p2pl error metric

    rt6D3 = MinimizingP2PLErrorMetric(transformedCloud3(corrs3.col(0), Eigen::all), tgtCloud(corrs3.col(1), Eigen::all), kinectNoiseWeights(corrs3.col(0), Eigen::all));
	meanweight3 = 1;
	}
	std::cout<<"corrs3num"<<std::endl;
	std::cout<<corrs3.rows()<<std::endl;
	//4th set of correspondences
	int cnt4[srcCloud.rows()];
    for(int i=0; i<srcCloud.rows(); i++)
	{
		//random
	...
	for(int i =0;i<srcCloud.rows(); i++)
	{
		if(cnt4[i]==1)
		{
			transformedCloud4(cnttrans4, 0) = transformedCloud(i, 0);
			transformedCloud4(cnttrans4, 1) = transformedCloud(i, 1);
			transformedCloud4(cnttrans4, 2) = transformedCloud(i, 2);
			transformedCloud4(cnttrans4, 3) = transformedCloud(i, 3);
			transformedCloud4(cnttrans4, 4) = transformedCloud(i, 4);
			transformedCloud4(cnttrans4, 5) = transformedCloud(i, 5);
	...
	Eigen::Vector<Scalar, 6> rt6D4;
		//match correspondence
    if (!MatchingByProject2DAndWalk4(transformedCloud4, tgtCloud)) {
    ...
    rt6D4 = MinimizingP2PLErrorMetric(transformedCloud4(corrs4.col(0), Eigen::all), tgtCloud(corrs4.col(1), Eigen::all), kinectNoiseWeights(corrs4.col(0), Eigen::all));
	std::cout<<"cloudnumber4"<<std::endl;
	std::cout<<srcCloud.rows()<<std::endl;
...
bool EAS_ICP::MatchingByProject2DAndWalk1(const SourceCloud& srcCloud, const TargetCloud& tgtCloud) {
  int size = srcCloud.rows();
  //correspondence declare
  std::vector<std::tuple<Scalar, int, int>> stdCorrs; 
  std::vector<double> residual_vector;
  ...
  correspondences.swap(corrs1);

  //check the correspondence size over 6 for solving 6 rt valuables
  return corrs1.rows() >=6;
}
bool EAS_ICP::MatchingByProject2DAndWalk2(const SourceCloud& srcCloud, const TargetCloud& tgtCloud) {
  int size = srcCloud.rows();
  //correspondence declare
  std::vector<std::tuple<Scalar, int, int>> stdCorrs; 
  std::vector<double> residual_vector;
  ...
  correspondences.swap(corrs2);

  //check the correspondence size over 6 for solving 6 rt valuables
  return corrs2.rows() >=6;
}
bool EAS_ICP::MatchingByProject2DAndWalk3(const SourceCloud& srcCloud, const TargetCloud& tgtCloud) {
  int size = srcCloud.rows();
  //correspondence declare
  std::vector<std::tuple<Scalar, int, int>> stdCorrs; 
  std::vector<double> residual_vector;
  ...
  correspondences.swap(corrs3);

  //check the correspondence size over 6 for solving 6 rt valuables
  return corrs3.rows() >=6;
}
bool EAS_ICP::MatchingByProject2DAndWalk4(const SourceCloud& srcCloud, const TargetCloud& tgtCloud) {
  int size = srcCloud.rows();
  //correspondence declare
  std::vector<std::tuple<Scalar, int, int>> stdCorrs; 
  std::vector<double> residual_vector;
  ...
  correspondences.swap(corrs4);

  //check the correspondence size over 6 for solving 6 rt valuables
  return corrs4.rows() >=6;
}
Eigen::Vector<EAS_ICP::Scalar, 6> EAS_ICP::MinimizingP2PLErrorMetric(const SourceCloud& srcCloud, const TargetCloud& tgtCloud, const Eigen::Matrix<Scalar, Eigen::Dynamic, 1>& weights) {
  ...
  cons_number = evalsolver.eigenvalues().maxCoeff()/evalsolver.eigenvalues().minCoeff();
  //evalsolver.eigenvalues().head(5);
  cons_number56 = evalsolver.eigenvalues().coeff(1)/evalsolver.eigenvalues().coeff(0);
  std::vector<int> inds;
