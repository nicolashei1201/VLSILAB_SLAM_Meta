...
void ICPORB_VO::Fuse(int n, bool orb_valid, bool icp_valid) {
  ...
  //for only one icp edge
    ...
    Eigen::MatrixX<Scalar> ma = Eigen::MatrixX<Scalar>::Identity(6,6);
    
    /*ma(0,0) = (0.8-(pICPVO->Sindex())*40);  
    ma(1,1) = (0.8-(pICPVO->Sindex())*40); 
    ma(2,2) = (0.8-(pICPVO->Sindex())*40); 
    ma(3,3) = (0.8-(pICPVO->Sindex())*40); 
    ma(4,4) = (0.8-(pICPVO->Sindex())*40); 
    ma(5,5) = (0.8-(pICPVO->Sindex())*40);*/
	//weighted
    /*if(pICPVO->Condition_number()<800){
	ma(0,0) = (1-(pICPVO->Condition_number())/800);  
    ma(1,1) = (1-(pICPVO->Condition_number())/800);  
    ma(2,2) = (1-(pICPVO->Condition_number())/800); 
    ma(3,3) = (1-(pICPVO->Condition_number())/800); 
    ma(4,4) = (1-(pICPVO->Condition_number())/800); 
    ma(5,5) = (1-(pICPVO->Condition_number())/800); 
	}
	else{
	ma(0,0) = 0;  
    ma(1,1) = 0; 
    ...
    ma(5,5) = 0;	 
  	...
  	if(pICPVO->Condition_number56()>3){
	ma(0,0) = ma(0,0)*0.8;  
    ma(1,1) = ma(1,1)*0.8; 
    ma(2,2) = ma(2,2)*0.8; 
    ma(3,3) = ma(3,3)*0.8; 
    ...
    //e->setInformation(Eigen::MatrixX<Scalar>::Zero(6,6));
    optimizer.addEdge(e);
  }
  // for (int i = 1; i < n; ++i) {
  //   g2o::EdgeSE3* e = new g2o::EdgeSE3();
  ...
  if (orb_valid) {
    ...
    Eigen::MatrixX<Scalar> ma = Eigen::MatrixX<Scalar>::Zero(6,6);
  ...
  printf("%f", pICPVO->Condition_number());
  printf("\n");
  printf("condition number56\n");
  printf("%f", pICPVO->Condition_number56());
  printf("\n");*/
}
...
auto TICP = fICP.get();
  ...
  //get ICP pose
  double slidingindex = pICPVO->Sindex();
  //printf("Sindex\n");
  //printf("%f\n", slidingindex);
  //printf("isvalid\n");
  //std::cout<<pICPVO->IsValid()<<std::endl;
...
//
void ICPORB_VO::IncrementalTrack(const cv::Mat& rgb, const cv::Mat& depth, const double& timestamp, const Eigen::Matrix4f& metapose) {
//clock_t t1, t2;
 //t1 =clock();
 //std::cout<<metapose.matrix()<<std::endl;
  auto fORB = std::async(
    std::launch::async,
    &ORB_SLAM2::System::TrackRGBD,
    pORBVO,
...
//std::cout<<double(t2-t1)/CLOCKS_PER_SEC<<std::endl;
  ...
  //convert Matrix4d to Isometry
  auto TICP = fICP.get();
  /*std::cout<<"metapose"<<std::endl;
  std::cout<<metapose<<std::endl;*/
  /*TICP(0,0) = metapose(0,0);
  TICP(0,1) = metapose(0,1);
  TICP(0,2) = metapose(0,2);
  TICP(0,3) = metapose(0,3);
  TICP(1,0) = metapose(1,0);
  TICP(1,1) = metapose(1,1);
  TICP(1,2) = metapose(1,2);
  TICP(1,3) = metapose(1,3);
  TICP(2,0) = metapose(2,0);
  TICP(2,1) = metapose(2,1);
  TICP(2,2) = metapose(2,2);
  TICP(2,3) = metapose(2,3);*/
  
  /*TICP(3,0) = metapose(3,0);
  TICP(3,1) = metapose(3,1);
  TICP(3,2) = metapose(3,2);
  TICP(3,3) = metapose(3,3);*/
  Pose PICP(TICP);
  //Pose PICP(metapose);
  //fuse both trajectory
  posesORB.push_back(PORB_inv.inverse());
